  ...
  return (
    <div className={`flex flex-col h-full space-y-5 bg-card/45 p-4 border border-border/40 rounded-2xl ${className}`}>
      ...
      <div className="flex-1 overflow-y-auto space-y-4 max-h-[360px] pr-1">
        {groups.map((group) => {
          const groupSessions = filteredSessions.filter((s) => s.dateGroup === group);
          if (groupSessions.length === 0) return null;

          return (
            <div key={group} className="space-y-1.5">
              <span className="text-[9px] font-bold text-muted-foreground/60 uppercase tracking-widest px-2">
                {group}
              </span>
