...
from app.database import EMISSION_FACTORS_DB, CARBON_TRANSACTIONS_DB, ENVIRONMENTAL_GOALS_DB, DEPARTMENTS_DB
...
@router.get("/transactions", response_model=APIResponse[List[CarbonTransactionResponse]])
...
@router.post("/transactions", response_model=APIResponse[CarbonTransactionResponse], status_code=status.HTTP_201_CREATED)
...
@router.put("/transactions/{id}", response_model=APIResponse[CarbonTransactionResponse])
...
@router.delete("/transactions/{id}", response_model=APIResponse[None])
